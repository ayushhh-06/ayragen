        ...
        <div className="w-full max-w-sm mx-auto">
          ...
          >
            <h2 className="text-3xl font-bold mb-3 tracking-tight font-display">
              {isForgot ? 'Reset Password' : isLogin ? 'Welcome back' : 'Start your journey'}
            </h2>
            <p className="text-[14px] text-white/30 font-light">
              {isForgot ? 'Enter your email to receive a reset link.' : isLogin ? 'Enter your credentials to enter the studio.' : 'Create an account to begin building your universe.'}
            </p>
          </motion.div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            {!isForgot && (
              <div>
                <div className="flex justify-between items-center mb-3">
                  ...
                  {isLogin && <button type="button" onClick={() => setIsForgot(true)} className="text-[11px] text-white/20 hover:text-white transition-colors">Forgot password?</button>}
                </div>
                <input 
                  type="password" 
                  name="password"
            ...
            {(error || message) && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`p-4 rounded-xl border text-xs ${error ? 'bg-red-500/10 border-red-500/20 text-red-400' : 'bg-purple-500/10 border-purple-500/20 text-purple-400'}`}>
                {error || message}
              </motion.div>
            )}
            ...
            <div className="pt-4">
              ...
              >
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : (isForgot ? 'Send Reset Link' : isLogin ? 'Log In Studio' : 'Create Account')}
            ...
            {isForgot && (
              <button 
                ...
                className="w-full text-center text-[12px] text-white/30 hover:text-white transition-colors"
              >
                Back to Login
              </button>
            )}
